...
import com.cb.service.UserExistsException;
	...
	public String onSubmit(User user, BindingResult result,
		...
		if (request.getParameter("delete") != null) {
			...
			request.getSession().setAttribute("message",
					getText("user.deleted", user.getUserFullName()));
		} else {
			try {
				user = userManager.saveUser(user);
				if (null != user) {
					System.out.println("user.getId()---------" + user.getId());
					User curUser = userManager.get(user.getId());
					if (null != curUser) {
						log.debug("curUser.getUserFullName()---------"
								+ curUser.getUserFullName());
						log.debug("curUser.getUsername()---------"
								+ curUser.getUsername());
					}
				}
			...
			if (null != user) {
				log.debug("user.getFullName()---------"
						+ user.getFullName());
				request.getSession().setAttribute("message",
						getText("user.saved", user.getUserFullName()));
			}
		}
